...
image.save(r'C:\Users\vkotr\New folder (2)\ChurnGuard\churnguard_transparent.png', "PNG")
...
col2.markdown('<p class="big-font" style="text-align: center; font-size: 30px;">Churn Guard</p>', unsafe_allow_html=True)
col2.markdown('<p class="small-font" style="text-align: center; font-size: 20px;">Locking in Loyalty</p>', unsafe_allow_html=True)
...
with col[0]:
    ...
    with st.form("my_form"):
        ...
        dependents = st.slider("Form slider", min_value=0, max_value=10, step=1)
        current_balance = st.number_input("Current Balance", step=1)
        previous_month_end_balance = st.number_input("Previous month end balance", step=1)
        average_monthly_balance_prevQ = st.number_input("Average monthly balance prevQ", step=1)
        average_monthly_balance_prevQ2 = st.number_input("Average monthly balance prevQ2", step=1)
        current_month_credit = st.number_input("Current month credit", step=1)
        previous_month_credit = st.number_input("Previous month credit", step=1)
        ...
        current_month_balance = st.number_input("Current month balance", step=1)
        previous_month_balance = st.number_input("Previous month balance", step=1)
        gender = st.selectbox("Gender", options=["Male", "Female", "Other"])
        occupation = st.selectbox("Occupation", options=["Retired", "Salaried", "Self Employed", "Student"])
        customer_Networth = st.selectbox("Customer Networth", options=['High', 'Medium', 'Low'])
        days_since_last_transaction = st.number_input("Days since last transaction", step=1)
...
with col[1]:
    st.markdown('#### Churn Rate by State')
    ...
    with st.expander('About', expanded=True):
        st.write('''
            ...
            - :orange[**Present Churn Rate**]: states with high inbound/ outbound migration for selected year
...
with col[2]:
    ...
    donut_chart_greater = make_donut(churn_lower, 'Forecasted Churn Rate', 'green')
